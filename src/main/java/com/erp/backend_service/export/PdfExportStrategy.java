...
/**
 * Hiện thực hóa chiến lược xuất báo cáo ra định dạng PDF dạng bảng khổ ngang (Landscape A4) bằng OpenPDF.
 */
@Component
public class PdfExportStrategy implements ExportStrategy {
    ...
    public byte[] export(ReportDataContext context) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            // Sử dụng khổ A4 nằm ngang để hiển thị tốt bảng nhiều cột
            Document document = new Document(PageSize.A4.rotate(), 20, 20, 25, 25);
            PdfWriter writer = PdfWriter.getInstance(document, out);

            document.open();

            // Font mặc định Helvetica
            ...
            // Độ rộng tương đối giữa các cột
